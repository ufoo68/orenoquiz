...
export const QuestionFormContents: FC<Props> = ({ contents, handleSave }) => {
  if (contents.type === 'select') {
    return <div className="flex flex-col space-y-5 items-center">
      <ul className="menu bg-base-100 p-2 rounded-box border w-max">
        {sortBy(contents.questions, 'id').map((question) =>
          <li key={question.id}>
            <a className="cursor-default">
              <input className="input input-bordered" value={question.label} onChange={(e) => {
                const otherQuestions = contents.questions.filter((q) => q.id !== question.id);
                handleSave({
                  ...contents,
                  questions: [
                    ...otherQuestions,
              ...
              {question.id === contents.answerId ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="green" className="w-6 h-6">
                <path fillRule="evenodd" d="M19.916 4.626a.75.75 0 01.208 1.04l-9 13.5a.75.75 0 01-1.154.114l-6-6a.75.75 0 011.06-1.06l5.353 5.353 8.493-12.739a.75.75 0 011.04-.208z" clipRule="evenodd" />
              </svg> : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="red" className="w-6 h-6 cursor-pointer" onClick={() => {
                handleSave({
                  ...contents,
              ...
              <svg
                ...
                onClick={() => {
                  handleSave({
                    ...contents,
                    questions: contents.questions.filter((q) => q.id !== question.id),
                  })
                }}
              >
