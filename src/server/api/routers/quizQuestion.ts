...
export const quizQuestionRouter = createTRPCRouter({
  ...
  getAll: publicProcedure.input(z.object({ masterId: z.string() }))
    .query(({ ctx, input }) => {
  ...
  create: publicProcedure.input(z.object({ masterId: z.string(), order: z.number() }))
    .mutation(async ({ ctx, input }) => {
      const { masterId, order } = input;
      await ctx.prisma.quizQuestion.create({
        data: { masterId, title: '新しい問題', contents: getSelectTypeInit(), order },
      });
    }),
  update: publicProcedure.input(z.object({ id: z.string(), title: z.string(), order: z.number(), contents: z.any() }))
    .mutation(async ({ ctx, input }) => {
  ...
  delete: publicProcedure.input(z.object({ questionId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const { questionId } = input;
